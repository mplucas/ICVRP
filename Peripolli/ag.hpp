...
class Genetic{
	...
	public:
		Genetic(int, int, int, int, int, int, char[], bool, int);//Completed
...
Genetic::Genetic(int population_size, int selection_rate, int cross_percent, int mutation_percent, int max_cross, int gen, char* file_source, bool backup, int description){
	switch(description){
		case 0:
			titles  = false;
			details = false;
...
int* Genetic::k_means(int k, int max_x, int max_y){
	...
	vector<clientNode> clusterClients;

	for( int i = 1; i < problem->client; i++ ){
		
		clientNode client;
		client.x = problem->info[i][0];
		client.y = problem->info[i][1];

		clusterClients.push_back(client);
	}

	sort(clusterClients.begin(), clusterClients.end(), xClientComparator);
	...
	for( int i = 1; i < problem->client; i++ ){
		...
		clusters[choosen_cluster_id].x += clusterClients[i].x;
		clusters[choosen_cluster_id].x /= 2;

		clusters[choosen_cluster_id].y += clusterClients[i].y;
...
double Genetic::breakRoute(bool printRoutes, int* major){
	...
	for(i=0; i<elements; i++){	
		...
		if( (timer + problem->cost[prev][dest] > problem->dueTime[dest]) ||
			...
			// printf("\n\nl timer %lf  problem->cost[prev][dest] %lf  problem->dueTime[dest] %lf", timer, problem->cost[prev][dest], problem->dueTime[dest]);
			// printf("\n\nl cap %i  problem->demand[dest] %i  problem->dueTime[dest] %i", capacity, problem->demand[dest], problem->capacity);
...
void Genetic::populate(){
	...
	if(readBackup){ // readBackup allways false
		...
		float randomic_pop_size = ceil(float(pop_size)*0.8); // 80%
		float directed_randomic_pop_size  = ceil(float(pop_size)*0.8); // 40%
		//the 20% missing is for kmeans
		...
		// directed randomic generation
		while(i < directed_randomic_pop_size){

			int* tmp = endTimeVector(problem, elements);

			fit = breakRoute(false, tmp);
			if(fit == -1 || !checkConsistency(tmp, elements) || i >= directed_randomic_pop_size){
				freeMemory(tmp);
			}else{
				if(fit<bestPrior || bestPrior<0) bestPrior = fit;
				generalFitness[i]=fit;
			...
			if(i < directed_randomic_pop_size) continue;
		...
		printf("\n\nL pop criada:\n");
		for(i=0; i<pop_size; i++){
			if(i == pop_size || i == randomic_pop_size)
				printf("---------------------------------------------------\n");
			for(int j=0; j<elements; j++){
				printf("[%i]: %i ", j, pop[i][j]);
			}
			printf("\n");
		}
	}
